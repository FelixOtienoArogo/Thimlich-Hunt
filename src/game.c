...
/* Wall texture image used for CPU-side pixel sampling */
Image wall_image;

/* Wall texture pixels used by the raycaster */
Color *wall_pixels;
...
void game_run(void)
{
    ...
    /* Load image so raycaster can read individual texture pixels */
    wall_image = LoadImage("assets/textures/Thim_tile.png");

    /* Store imave pixels for manual texture sampling */
    wall_pixels = LoadImageColors(wall_image);

    /* Still create texture in case we need GPU rendering later*/
    wall_texture = LoadTextureFromImage(wall_image);

    /* Keep texture sharp and avaoid blurry stretching */
    ...
    /* Release texture pixel data from memory*/
    UnloadImageColors(wall_pixels);
